...
class NatalChartViewModel {
    ...
    private func parseAspects(from chart: Chart?) -> [AstrologicalAspectData] {
        ...
        return chart.aspects.compactMap { aspectDescription in
            let components = aspectDescription.split(separator: " ")
            guard components.count >= 7 else { return nil }

            let planet1Str = String(components[0])
            let aspectTypeStr = components[1].lowercased()
            let planet2Str = String(components[2])
            let exactAngleStr = components[4].trimmingCharacters(in: CharacterSet(charactersIn: "°"))
            let orbStr = components[6].trimmingCharacters(in: CharacterSet(charactersIn: "°"))

            guard let planet1 = Point(rawValue: planet1Str),
                  let planet2 = Point(rawValue: planet2Str),
                  let exactAngle = Double(exactAngleStr),
                  let orb = Double(orbStr),
                  let aspectType = Aspect.allCases.first(where: { $0.description.lowercased() == aspectTypeStr }) else {
                return nil
            }
